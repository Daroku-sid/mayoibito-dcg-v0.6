...
const Screens = {
  ...
  _render: function () {
    ...
    /* 角の設定ボタンは、モード選択のときだけ出します（v0.5.6）。
       画面の浮かび上がる演出の外に置いてあるので、
       ここで出し入れを決めます。 */
    const corner = document.getElementById('corner-options');
    if (corner) corner.classList.toggle('is-hidden', name !== 'mode');
  ...
  _startSoloMatch: function () {
    ...
    s.deck1 = DeckPickerUI.ensureUsable(s.deck1, 'village');
    s.deck2 = DeckPickerUI.ensureUsable(s.deck2, 'mansion');
  ...
  _startCpuMatch: function () {
    ...
    // 選んだデッキが消えていたり使えなくなっていたら、公式へ戻します
    c.playerDeck = DeckPickerUI.ensureUsable(c.playerDeck, 'village');
