...
console.log('\n■ チュートリアルの起動口（v0.5.1で踏んだ不具合の再発防止）');
{
  const prev = fs.readFileSync('js/preview.js', 'utf8');

  /* v0.5.0 では Screens.go('play') を呼んでいました。
     'play' という画面は存在せず、メニューが開いたまま盤面が見えませんでした。
     対戦画面はメニューの層を閉じると下から現れます。 */
  const names = [];
  html.replace(/data-screen="([^"]+)"/g, function (m, n) { names.push(n); return m; });
  check('★存在しない画面へ飛ばしていない',
    !/Screens\.go\('play'\)/.test(prev),
    "Screens.go('play') は画面が無いので使えません");
  check('画面名の一覧に play が無いことを確認', names.indexOf('play') === -1);

  const entry = (prev.match(/startTutorialGame[\s\S]{0,700}/) || [''])[0];
  check('★チュートリアルはメニュー層を閉じてから始める',
    /Screens\.close\(\)/.test(entry));
  check('★カード画像の先読みを通す（通常の対戦と同じ）',
    /startWithLoading/.test(entry));
  check('CPUの設定を渡している（渡さないと相手が動かない）',
    /cpu:\s*opts\.cpu/.test(entry));
}
...
console.log('\n■ 遊べないモードが無いか（画面の行き先）');
{
  ...
  //   tutorial-finish … 実践編をクリアした瞬間だけ出る（仕様書 21.8）
  const ROOTS = ['start', 'mode', 'deck-view', 'deck-edit', 'field-select', 'deck-pick',
                 'tutorial-finish'];
